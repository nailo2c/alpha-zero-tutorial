...
class Game:
    def __init__(self):
        ...
        self.grid_shape = (6,7)
        self.input_shape = (2,6,7)  # 兩個玩家各一個盤面
        self.name = 'connect4'
    ...
    def step(self, action):
        ...
        self.gameState = next_state  # next_state 不用用gameState包起來嗎?
...
class GameState:
    def __init__(self, board: np.array, playerTurn):
        self.board = board  # shape: (42,)
        self.pieces = {'1':'X', '0':'-', '-1':'O'}
        # 窮舉所有勝利狀況
        self.winners = [
            [0,1,2,3],
        ...
        for i in range(len(self.board)):
            ...
            # 剩下的要下方位有擺放子才允許
                ...
                if self.board[i] == 0 and self.board[i+7] != 0:
                    allowed.append(i)
        # 返回允許擺放的index, ex: [0, 3, 17, ...]
        return allowed

    def _binary(self) -> list:
        """取得雙方盤面位置(相對視角)"""
        currentplayer_position = np.zeros(len(self.board), dtype=np.int)
        currentplayer_position[self.board==self.playerTurn] = 1

        other_postition = np.zeros(len(self.board), dtype=np.int)
        other_postition[self.board==-self.playerTurn] = 1

        position = np.append(currentplayer_position, other_postition)

        return position

    def _converStateToId(self) -> str:
        """轉化盤面為str(絕對視角)"""
        player1_position = np.zeros(len(self.board), dtype=np.int)
        player1_position[self.board==1] = 1

        other_postition = np.zeros(len(self.board), dtype=np.int)
        other_postition[self.board==-1] = 1

        position = np.append(player1_position, other_postition)
    ...
    def takeAction(self, action):
        ...
        if newState.isEndGame:
            value = newState.value[0]  # 結束就得到 -1
            done = 1
